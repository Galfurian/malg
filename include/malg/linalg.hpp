...
/// the input matrix.
template <typename T>
inline auto cofactor(const MatrixBase<T> &matrix, unsigned p, unsigned q)
{
...
inline auto qr_decomposition(const Matrix<T> &A)
{
    ...
    // Compute Q and R.
    for (unsigned i = 0; i < (R.cols() - (R.cols() == R.rows())); ++i) {
        // Initialize holder.
        for (unsigned r = 0; r < H.rows(); ++r)
            for (unsigned c = 0; c < H.cols(); ++c)
                H(r, c) = (r == c) ? 1 : 0;
    ...
    for (unsigned r = 1; r < R.rows(); ++r)
        for (unsigned c = 0; c < std::min(r, R.cols()); ++c)
            R(r, c) = 0;
...
auto qr_decomposition(const malg::Matrix<DT> &A)
{
    ...
    double length;
    for (unsigned r = 1, c = 0; r < A.rows(); ++r, c = r - 1) {
        // r : The row number
